...
def test_find_by_name_returns_all_windows_with_same_name(
    mw_store: MWStore, scheduling_context: SchedulingContext
) -> None:
    a_windows = [
    ...
    with ssm_returning(a_windows + b_windows):
        mw_context = new_mw_context(
            mw_store,
            scheduling_context,
            mws_referenced_by_schedules=a_windows + b_windows,
        )
...
def test_contains_returns_false_on_empty_context(
    ...
    mw_context = new_mw_context(mw_store, scheduling_context)
...
def test_contains_returns_true_when_name_has_at_least_1_associated_mw(
    ...
    with ssm_returning(windows):
        mw_context = new_mw_context(
            mw_store, scheduling_context, mws_referenced_by_schedules=windows
        )
...
def test_reconcile_adds_new_mws_to_db(
    mw_store: MWStore, scheduling_context: SchedulingContext
) -> None:
    with ssm_returning([new_mw("mw-00000000000000000")]):
        new_mw_context(mw_store, scheduling_context).reconcile_ssm_with_dynamodb()

        db_mws = list(
            mw_store.find_by_account_region(
                scheduling_context.assumed_role.account,
                scheduling_context.assumed_role.region,
            )
        )
        assert len(db_mws) == 1
...
def test_reconcile_filters_out_windows_not_referenced_by_at_least_one_schedule(
    ...
    with ssm_returning([referenced_window, not_referenced_window]):
        new_mw_context(
            mw_store,
            scheduling_context,
            mws_referenced_by_schedules=[referenced_window],
...
def test_reconcile_updates_mws_in_db_when_existing_window_is_not_active(
    ...
    with ssm_returning([next_window]):
        new_mw_context(
            mw_store, scheduling_context, current_dt=quick_time(15, 0, 0)
        ).reconcile_ssm_with_dynamodb()
...
def test_reconcile_does_not_delete_windows_that_are_still_running_until_they_stop(
    ...
    with ssm_returning([]):
        # window running
        new_mw_context(
            mw_store, scheduling_context, current_dt=quick_time(11, 0, 0)
        ).reconcile_ssm_with_dynamodb()
...
def test_reconcile_does_not_overwrite_windows_that_are_still_running_until_they_stop(
    ...
    with ssm_returning([next_window]):
        ...
        assert running_window in list(
            mw_store.find_by_account_region(
                scheduling_context.assumed_role.account,
                scheduling_context.assumed_role.region,
            )
        )
        assert next_window not in list(
            mw_store.find_by_account_region(
                scheduling_context.assumed_role.account,
                scheduling_context.assumed_role.region,
            )
...
def new_mw_context(
    ...
    if current_dt is not None:
        scheduling_context.current_dt = current_dt

    if mws_referenced_by_schedules is not None:
        mw_schedule = ScheduleDefinition(
            name="test-schedule",
            ssm_maintenance_window=[
                mw.window_name for mw in mws_referenced_by_schedules
            ],
            timezone="UTC",
        )
        scheduling_context.schedule_store.put(mw_schedule, True)
    else:
        # Ensure there's always at least one schedule for default behavior
        default_schedule = ScheduleDefinition(
            name="default-schedule",
            ssm_maintenance_window=["test-window"],
            timezone="UTC",
        )
        scheduling_context.schedule_store.put(default_schedule, True)
