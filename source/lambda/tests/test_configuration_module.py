    ...
    for schedule_name in response.schedules:
        if schedule_name == 'docDbSchedule':
            ...
            for period in response.get_schedule(schedule_name).periods:
                ...
                assert period.get("period").get_desired_state(
                    logger, valid_current_datetime) == "running"

                invalid_datetime = datetime.datetime(
                    2021, 6, 20, 17, 30, 59, 34258)
                assert period.get("period").get_desired_state(
                    logger, invalid_datetime) == "stopped"
        ...
        if schedule_name == 'monstartandfridayend':
            ...
            for period in response.get_schedule(schedule_name).periods:
                if period.get("period").name == 'tuethu':
                    ...
                    assert period.get("period").get_desired_state(logger, tuesday_nine_am) == "running"
                if period.get("period").name == 'fridaystop':
                    friday_five_fifteen_pm = datetime.datetime(2021, 9, 17, 17, 15, 00, 0)
                    assert period.get("period").get_desired_state(logger, friday_five_fifteen_pm) == "stopped"
                    friday_four_fortyfive_pm = datetime.datetime(2021, 9, 17, 16, 45, 00, 0)
                    assert period.get("period").get_desired_state(logger, friday_four_fortyfive_pm) == "running"
                    sat_four_fortyfive_pm = datetime.datetime(2021, 9, 18, 16, 45, 00, 0)
                    assert period.get("period").get_desired_state(logger, sat_four_fortyfive_pm) == "stopped"
                    sunday_four_fortyfive_pm = datetime.datetime(2021, 9, 19, 16, 45, 00, 0)
                    assert period.get("period").get_desired_state(logger, sunday_four_fortyfive_pm) == "stopped"
