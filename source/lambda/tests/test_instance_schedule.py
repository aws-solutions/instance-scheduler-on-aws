...
def test_instance_schedule():
    ...
    logstream = LOG_STREAM.format(
        'test_instance_schedule', dt.year, dt.month, dt.day)
    ...
    ec2Instance = collections.namedtuple(
        'ec2Instance', 'id schedule_name name state state_name is_running is_terminated current_state instancetype tags account region service instance_str')
    instance = ec2Instance(id='i-00bedf8a12df1dd6a', schedule_name='stopped', name='ISRelated', state=16, state_name='running', is_running=True, is_terminated=False, current_state='running',
                           instancetype='t2.micro', tags={'Name': 'ISRelated', 'Schedule': 'running'}, account='111111111111', region='us-east-1', service='ec2', instance_str='EC2:i-00bedf8a12df1dd6a (ISRelated)')
    try:
        ...
        if current_test_execution_time.today().weekday() in [0, 1, 2, 3, 4]:
            if current_test_execution_time.hour < 17 and current_test_execution_time.hour > 9:
