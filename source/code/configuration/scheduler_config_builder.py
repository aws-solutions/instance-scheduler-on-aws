...
MSG_BEGIN_MUST_BEORE_END = "Begin time {} must be earlier than end time in {}"
...
MSG_OVERWRITE_OVERRIDE_MUTALLY_EXCLUSIVE = "{} and {} are mytualy exclusive options"
...
class SchedulerConfigBuilder:
    ...
    def build(self, config):
        """
        Builds the configuration instance from the raw configuration data
        :param config: raw configuration data returned from storage adapter
        ...
        try:
            return SchedulerConfig(scheduled_services=config.get(configuration.SCHEDULED_SERVICES, []) or [],
                                   ...
                                   schedules=self._build_schedules(config, get_default_timezone(config)),
                                   trace=config.get(configuration.TRACE, False),
                                   use_metrics=config.get(configuration.METRICS, False),
                                   cross_account_roles=config.get(configuration.CROSS_ACCOUNT_ROLES, []) or [],
    ...
    def _build_schedules(self, conf, dflt_tz):
        schedules = {}
        ...
        for schedule_config in conf.get(configuration.SCHEDULES, []):
            schedule = self._build_schedule(schedule_config, dflt_tz)
            if schedule is None:
                continue
    ...
    def _build_schedule(self, schedule_config, dflt_tz):
        ...
        def get_override_status(config):
            if configuration.OVERWRITE in config:
                if configuration.OVERRIDE_STATUS in config:
                    raise ValueError(
                        MSG_OVERWRITE_OVERRIDE_MUTALLY_EXCLUSIVE.format(configuration.OVERWRITE, configuration.OVERRIDE_STATUS))
        ...
        try:
            ...
            # ignore periods if there is an always on or if overrride_status option is used
            if not override_status:
                # use current date and time for timezone of schedule
                current_schema_dt = datetime.datetime.now(SchedulerConfigBuilder._get_timezone(timezone))
            ...
            return InstanceSchedule(name=get_schedule_name(schedule_config),
                                    ...
                                    use_metrics=schedule_config.get(configuration.METRICS, False),
    ...
    def get_time_from_string(timestr):
        """
        Standarised method to build time object instance from time string
    ...
    def _build_period(cls, period_configuration, dt):
        ...
        def get_begin_and_end(period_name, config):
            ...
            if (begin and end) and begin > end:
                raise ValueError(MSG_BEGIN_MUST_BEORE_END.format(begin, end, period_name))
            return begin, end
